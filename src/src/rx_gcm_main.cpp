...
void ICACHE_RAM_ATTR TXdoneCallback() {
  if (!RxHandshake.IsDone()) RxHandshake.TXdoneCallback();
}

bool ICACHE_RAM_ATTR
RXdoneCallback(SX12xxDriverCommon::rx_status const status) {
  if (!RxHandshake.IsDone()) RxHandshake.RXdoneCallback(status);
  return true;
}
...
void setup() {
  ...
  // Radio.Config(SX1280_LORA_BW_0800, SX1280_LORA_SF8, SX1280_LORA_CR_LI_4_8,
  // 2420000000, 12, false, 8, 20000, 0, 0, 0);
