...
void ICACHE_RAM_ATTR TXdoneCallback() {
  if (!TxHandshake.IsDone()) TxHandshake.TXdoneCallback();
}

bool ICACHE_RAM_ATTR
RXdoneCallback(SX12xxDriverCommon::rx_status const status) {
  if (!TxHandshake.IsDone()) TxHandshake.RXdoneCallback(status);
  return true;
}
