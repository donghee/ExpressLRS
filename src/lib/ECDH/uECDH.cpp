...
void ECDH::init()
{
    ...
    // generate_secret_key((const char *)compressed_public_key2_, compressed_public_key2_len);
    //
    // DebugSerial.print("ECC security key 1: ");
    // for (size_t i = 0; i < sizeof(secret_key_); i++) {
    //     DebugSerial.print(secret_key_[i], HEX);
    // }
    // DebugSerial.println();
    //
    // ECC_shared_secret(public_key_, private_key2_, secret_key2_, curves_);
    // DebugSerial.print("ECC security key 2: ");
    // for (size_t i = 0; i < sizeof(secret_key2_); i++) {
    //     DebugSerial.print(secret_key2_[i], HEX);
    // }
    // DebugSerial.println();
}
...
int ECDH::compress_public_key(unsigned char *to_compressed_public_key, size_t *to_compressed_public_key_len)
{
    ...
    *to_compressed_public_key_len = sizeof(to_compressed_public_key);
    return 1; // Success
}
...
int ECDH::compress_other_public_key(const char* from_public_key, size_t from_public_key_len, unsigned char *to_compressed_public_key, size_t *to_compressed_public_key_len)
{
    ...
    ECC_compress_public_key((uint8_t *)from_public_key, to_compressed_public_key, curves_);
    *to_compressed_public_key_len = sizeof(to_compressed_public_key);
    return 1; // Success
}

int ECDH::generate_secret_key(const char *compressed_public_key, size_t compressed_public_key_len)
{
    int ret = 1;
